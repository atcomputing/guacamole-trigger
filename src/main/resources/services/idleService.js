  ...
  function idleService($timeout, $document) {
    ...
    function wakeup (){
      ...
      if (waitingFor < idleCallbacks.length ){
        TimeOut_Thread = $timeout(wakeup, idleCallbacks[waitingFor].time - idleCallbacks[waitingFor-1].time );
      }
    }
    service.idleCallback = function (idleTime,idleCallback,activeCallback) {
      if (!TimeOut_Thread){

        TimeOut_Thread = $timeout(wakeup(), idleTime);
    ...
    service.resetCountDown = function resetCountDown(){
      ...
      /// Reset the timeout
      TimeOut_Thread = $timeout(wakeup, idleCallbacks[0].time);
      while (waitingFor > 0){
        waitingFor -=1;
